...
public class UserController {
    ...
    @ResponseStatus(HttpStatus.OK)
    @ApiOperation(value = "Get all users by address with paging ang sorting")
    @GetMapping(value = "/getByAddress/{address}")
    public List<UserDto> getUsersByUserAddress(@RequestParam(defaultValue = "Lviv") String address, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "3") int size) {
    System.out.println(address);
        return userService.getUsersByUserAddress(address, page, size);
    }
}
